...
try:
    from ..ai.ai_task_orchestrator import (
        AITaskOrchestrator,
...
class MermaidDiagramGenerator:
    ...
    async def generate_orchestrator_diagram(self, orchestrator_description: Dict[str, Any]) -> str:
        ...
        try:
            ...
            orchestrator_description.get('workflows', [])
    ...
    async def _generate_class_diagram(self, structure: Dict[str, Any], direction: str = "TD") -> str:
        ...
        for comp in structure.get('components', []):
            if comp.get('type') == 'class':
                ...
                diagram.append("        +method1()")
                diagram.append("        +method2()")
                diagram.append("    }")
    ...
    async def _generate_er_diagram(self, structure: Dict[str, Any], direction: str = "TD") -> str:
        ...
        for comp in structure.get('components', []):
            if comp.get('type') in ['database', 'table', 'entity']:
                ...
                diagram.append("        datetime created_at")
                diagram.append("    }")
...
- Comprehensive documentation generation"""
        ...
        # Components section
        components_text = []
        for _comp_id, comp_info in orchestrator_description.get('components', {}).items():
...
async def main():
    ...
    await context_optimizer.optimize_context({
