...
class SchemaManager:
    ...
    def load_schema_content(self, schema_info: SchemaInfo) -> Dict[str, Any]:
        ...
        try:
            with open(schema_info.file_path) as f:
                return json.load(f)
        except Exception as e:
    ...
    try:
        ...
        with console.status("[bold green]Creating schema...", spinner="dots"):
            try:
                ...
                # Show schema preview
                if dry_run:
                    console.print("[bold]Schema that would be created:[/bold]")
    ...
    try:
        ...
        with console.status("[bold green]Creating subtype schema...", spinner="dots"):
            try:
                ...
                # Success message
                console.print("[green]✅ Subtype schema created successfully![/green]")
                console.print(f"[dim]Name: {name}[/dim]")
...
def modify_schema(ctx, schema_id, add_property, remove_property, set_description,
    ...
    try:
        ...
        if not current_schema:
            console.print("[red]❌ Failed to load schema content[/red]")
            sys.exit(1)
        ...
        console.print("[green]✅ Schema modified successfully![/green]")
    ...
    try:
        ...
        with console.status("[bold green]Creating new version...", spinner="dots"):
            try:
                ...
                console.print("[green]✅ New schema version created![/green]")
...
def diff_schemas(ctx, schema_id_1, schema_id_2, output_format, context):
    ...
    try:
        ...
        if output_format == 'json':
            ...
            console.print("[bold]Comparing schemas:[/bold]")
    ...
    try:
        ...
        if instance_file:
            ...
            if validation_errors:
                console.print("[red]❌ Instance validation failed:[/red]")
                for error in validation_errors:
                    console.print(f"  • {error}")
                sys.exit(1)
            else:
                console.print("[green]✅ Instance validation passed![/green]")
