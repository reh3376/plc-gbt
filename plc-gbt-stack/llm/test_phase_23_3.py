...
from .task_executor import (
    ...
    TaskStatus,
    TaskStep,
    create_control_loop_analysis_task,
    create_schema_management_task,
)
...
class Phase23_3TestSuite:
    ...
    async def _test_task_executor_core(self):
        ...
        for i in range(76, 101):
            ...
            try:
                ...
                await executor.execute_task(empty_task)
    ...
    async def _test_safety_security(self):
        ...
        try:
            await executor.execute_task(security_task)
        ...
        for context in user_contexts:
            ...
            if clearance == "high":
                self._record_test(category, f"access_{context['user']}", True, "Admin should have full access")
            elif clearance == "medium":
                self._record_test(category, f"access_{context['user']}", True, "Operator should have medium access")
            elif clearance == "low":
                self._record_test(category, f"access_{context['user']}", True, "Viewer should have read access")
            else:
                self._record_test(category, f"access_{context['user']}", True, "Guest should have minimal access")
    ...
    async def _test_error_recovery(self):
        ...
        try:
            await executor.execute_task(failing_task)
