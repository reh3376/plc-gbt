    ...
    def test_configuration_management(self) -> List[TestResult]:
        ...
        if success and "Current Configuration" in stdout:
            ...
            results.append(TestResult(
                ...
                score=100.0,
                duration_seconds=duration,
                details=["Configuration displayed successfully", f"Found {len(found_items)}/{len(config_items)} expected items"]
            ))
        else:
            results.append(TestResult(
                test_name="Configuration Show",
        ...
        if users_file.exists():
            try:
                ...
                results.append(TestResult(
                    ...
                    score=100.0,
                    duration_seconds=0.0,
                    details=["Default users file created", f"Found users: {', '.join(found_users)}"],
            ...
            for category_name, test_function in test_categories:
                ...
                try:
                    ...
                    all_results.append(TestResult(
                        ...
                        score=0.0,
                        duration_seconds=0.0,
                        details=["Category testing failed with exception"],
                        error_message=str(e)
                    ))
...
async def main():
    ...
    try:
        ...
        # Display results
        console.print("\n" + "="*80)
        console.print("[bold]📊 PHASE 21.1 TESTING RESULTS[/bold]")
        console.print("="*80)
