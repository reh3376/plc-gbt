...
async def diagnose_neo4j_storage_issues():
    ...
    try:
        ...
        print(f"\n❌ Issue Identified:")
        print(f"   • Expected nodes from recent ingestion: 114 files")
        print(f"   • Actual PythonFile nodes in Neo4j: {python_files}")
        print(f"   • Missing nodes: {114 - python_files}")
        print(f"   • Root cause: DateTime serialization errors during storage")
...
async def test_fixed_storage():
    ...
    try:
        ...
        print(f"🔧 Original metadata (with datetime objects):")
        ...
        try:
            ...
            if result.success:
                print(f"✅ Neo4j storage test: SUCCESS")
...
async def recommend_plc_repo_ingestion():
    ...
    print(f"\n🔧 Recommended Actions:")
    print(f"   1. Clone all PLC repositories to local directory")
    print(f"   2. Update CLI to support multi-directory ingestion")
    print(f"   3. Run comprehensive ingestion across all PLC repositories")
    print(f"   4. Validate complete knowledge graph with all PLC data")
    
    print(f"\n💻 Example Commands:")
    print(f"   # Clone PLC repositories")
    print(f"   cd /Users/reh3376/repos/")
    for repo_name, repo_url in plc_repos.items():
        print(f"   git clone {repo_url}")
    
    print(f"\n   # Ingest all PLC repositories")
    print(f"   plc-memory ingest --directories plc-100 plc-200 plc-300 plc-400 plc-500 plc-600")
    print(f"   # Or ingest each individually")
...
'''
    ...
    print(f"📋 Next steps:")
    print(f"   1. Apply patch to file_processors.py")
    print(f"   2. Re-run ingestion with fixed storage")
    print(f"   3. Validate all 114 files are stored in Neo4j")
...
async def main():
    ...
    try:
        ...
        print(f"\n🎯 Diagnosis Complete!")
        print(f"📊 Summary of Issues Found:")
        print(f"   ❌ DateTime serialization preventing Neo4j storage")
        print(f"   ❌ Missing PLC repositories (plc-100 through plc-600)")
        print(f"   ❌ Only 73/114 files successfully stored in Neo4j")
        
        print(f"\n✅ Solutions Provided:")
        print(f"   🔧 DateTime serialization fix implemented")
        print(f"   📋 PLC repository cloning recommendations")
        print(f"   🔨 Storage fix patch created")
        
        print(f"\n🚀 Next Steps:")
        print(f"   1. Apply storage fixes to file_processors.py")
        print(f"   2. Clone missing PLC repositories")
        print(f"   3. Re-run comprehensive ingestion")
        print(f"   4. Validate complete Neo4j population")
