...
from scanner import ScanResult, SchemaAnalysis, CodeAnalysis, DocumentAnalysis, DataAnalysis
...
class ContentAnalyzer:
    ...
    def _find_property_patterns(self, schema_results: List[ScanResult]) -> List[SchemaPattern]:
        ...
        for result in schema_results:
            try:
                with open(result.metadata.path, 'r') as f:
    ...
    def _compare_schemas(self, result1: ScanResult, result2: ScanResult) -> Optional[ControlRelationship]:
        ...
        try:
            # Load both schemas
            with open(result1.metadata.path, 'r') as f:
                schema1 = json.load(f)
            with open(result2.metadata.path, 'r') as f:
                schema2 = json.load(f)
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if schema_count > 0:
            ...
            if len(complexity_levels) >= 3:
                recommendations.append(f"Multiple complexity levels - enables progressive learning approach")
...
def main():
    ...
    if args.scan_output:
        ...
        with open(args.scan_output, 'r') as f:
