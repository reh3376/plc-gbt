...
export function SystemHealthDashboard({
  ...
  const currentHealth = (systemHealth as unknown as SystemHealth) || mockSystemHealth
  ...
  const criticalComponents = useMemo(() =>
    currentHealth.components.filter(component => component.status === 'critical').length,
  ...
  const overallHealthStyle = getHealthStatusStyle(currentHealth.overall)
            ...
            {currentHealth.alerts.length === 0 ? (
              ...
              currentHealth.alerts.map((alert) => {
                const severityStyle = getHealthStatusStyle(
                  alert.severity === 'info' ? 'healthy' :
                  alert.severity === 'warning' ? 'warning' :
                  alert.severity === 'error' || alert.severity === 'critical' ? 'critical' :
                  'unknown'
                )
    ...
    <div className={cn("space-y-4", className)}>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-4 text-sm text-[#969696]">
              <div className="text-center">
                <div className="font-medium text-[#cccccc]">{currentHealth.metrics.uptime.toFixed(1)}%</div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { 
            label: 'CPU Usage', 
            value: currentHealth.metrics.cpuUsage, 
            unit: '%', 
            icon: Cpu,
          ...
          { 
                  label: 'Memory Usage',
      value: currentHealth.metrics.memoryUsage,
      unit: '%',
      icon: HardDrive,
          ...
          { 
            label: 'Disk Usage', 
            value: currentHealth.metrics.diskUsage, 
            unit: '%', 
            icon: HardDrive,
          ...
          { 
            label: 'Response Time', 
            value: currentHealth.metrics.avgResponseTime, 
            unit: 'ms', 
            icon: Activity,
