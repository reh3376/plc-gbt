...
import { useLayoutStore, type IconConfig, type ToolType } from '@/lib/stores/layout-store';
  ...
  return (
    <>
      ...
      >
        ...
        >
          <GripVertical className="w-3 h-3 text-white/90" aria-hidden="true" />
        </div>
...
export function IconStrip() {
  ...
  const allMainIcons = [...orderedIcons, ...customIcons.filter(icon => icon.position !== 'bottom')];
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (active.id !== over?.id) {
      ...
      if (oldIndex !== -1 && newIndex !== -1) {
        ...
        setAnnouncement(
          `${draggedIcon?.name} moved ${oldIndex < newIndex ? 'down' : 'up'} in toolbar, now positioned ${newIndex < oldIndex ? 'before' : 'after'} ${targetIcon?.name}`
        );
      }
    } else {
    ...
    >
      ...
      >
        ...
        <div id="icon-strip-instructions" className="sr-only">
          Use arrow keys to navigate between tools. Press Enter to activate a tool. Use Space and
          arrow keys to reorder tools when drag mode is active.
        </div>
