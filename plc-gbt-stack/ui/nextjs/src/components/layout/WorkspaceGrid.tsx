...
export function WorkspaceGrid() {
  ...
  const setSidebarIntegrated = useAIAssistantStore(state => state.setSidebarIntegrated);
  ...
  useEffect(() => {
    const checkPanelWidth = () => {
      if (leftColWidth > 80) {
        console.warn(
          'DEFENSIVE: Left panel width exceeded 80%, resetting from',
          leftColWidth,
          'to 80%'
        );
        setLeftColWidth(80);
    ...
    const emergencyCorrection = () => {
      ...
      if (column2Element || column2Handle) {
        if (!aiAssistant.isOpen || aiAssistant.position !== 'column-2') {
          ...
          console.error('Current AI state:', {
            isOpen: aiAssistant.isOpen,
            position: aiAssistant.position,
          });
          ...
          setTimeout(() => {
            const stillExists =
              document.querySelector('#ai-column-2') || document.querySelector('#column2-handle');
            if (stillExists) {
      ...
      if (layoutStorage) {
        try {
          ...
          if (parsed.state?.aiAssistant?.position === 'column-2') {
            console.error(
              'EMERGENCY: Found Column 2 in localStorage on mount, clearing immediately'
            );
            localStorage.removeItem('plc-gbt-layout-storage');
