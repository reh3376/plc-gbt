...
const algorithmFormSchema = z.enum(['dependent', 'independent', 'parallel', 'series']);

// Control Action Schema
const controlActionSchema = z.enum(['direct', 'reverse']);

// Process Variable Schema
const processVariableSchema = z
  .object({
    tag_name: z
      .string()
      ...
      .regex(
        /^[A-Za-z][A-Za-z0-9_]*$/,
        'Tag name must start with letter and contain only alphanumeric characters and underscores'
      ),
...
const controlOutputSchema = z
  .object({
    ...
    range_min: z
      .number()
      .min(0, 'Output range minimum cannot be negative')
      .max(100, 'Output range minimum cannot exceed 100%'),
    range_max: z
      .number()
      .min(0, 'Output range maximum cannot be negative')
      .max(100, 'Output range maximum cannot exceed 100%'),
    initial_value: z
      .number()
      .min(0, 'Initial value cannot be negative')
      .max(100, 'Initial value cannot exceed 100%'),
...
const scalingConfigurationSchema = z
  .object({
...
const cascadeConfigurationSchema = z
  .object({
...
const enhancedControlLoopSchema = z.object({
  ...
  schema_version: z
    .string()
    .regex(/^\d{2}\.\d{2}\.\d{3}$/, 'Schema version must follow XX.YY.ZZZ format'),
});
...
const dashboardFiltersSchema = z.object({
  status: z
    .array(z.enum(['running', 'stopped', 'error', 'tuning', 'manual', 'cascade']))
    .optional(),
...
export {
  ...
  setpointSchema,
  tuningInterfaceEventSchema,
  tuningInterfaceEventTypeSchema,
  // Section 1: Control Loop Tuning Interface Schemas
  tuningMethodSchema,
  tuningQueueActionSchema,
  tuningQueueActionTypeSchema,
  tuningQueueContextActionSchema,
  tuningQueueContextOptionSchema,
...
// ===== SECTION 1: Control Loop Tuning Interface Schemas =====
// Following AI Task Orchestrator TypeScript methodology - Strict Zod validation

// Tuning Method Schema
const tuningMethodSchema = z.enum([
  'manual',
  'ziegler_nichols',
  'cohen_coon',
...
// Control Loop Operating Mode Schema
const controlLoopOperatingModeSchema = z.enum(['Auto', 'Manual', 'Software Manual', 'Off']);

// Safety Limits Configuration Schema
const safetyLimitsConfigSchema = z.object({
  maxSetpointChange: z
    .number()
    .positive('Maximum setpoint change must be positive')
    .max(1000, 'Maximum setpoint change cannot exceed 1000'),
  maxOutputChange: z
    .number()
    .positive('Maximum output change must be positive')
    .max(100, 'Maximum output change cannot exceed 100%'),
  emergencyShutdownThreshold: z.number().positive('Emergency shutdown threshold must be positive'),
  parameterChangeRateLimit: z
    .number()
    .positive('Parameter change rate limit must be positive')
    .max(10, 'Rate limit cannot exceed 10 changes per second'),
});

// Advanced Settings Configuration Schema
const advancedSettingsConfigurationSchema = z.object({
  ...
  tuningAlgorithm: tuningMethodSchema,
  safetyLimits: safetyLimitsConfigSchema,
  historicalDataRetention: z
    .number()
    .int('Historical data retention must be an integer')
    .min(1, 'Must retain data for at least 1 day')
    .max(365, 'Cannot retain data for more than 365 days'),
  autoSaveInterval: z
    .number()
    .int('Auto-save interval must be an integer')
    .min(1000, 'Auto-save interval must be at least 1 second')
    .max(300000, 'Auto-save interval cannot exceed 5 minutes'),
});

// Focus Loop Editable Parameters Schema
const focusLoopEditableParametersSchema = z.object({
  setpoint: z
    .number()
  ...
  proportionalGain: z
    .number()
    .positive('Proportional gain must be positive')
    .max(999.9, 'Proportional gain cannot exceed 999.9'),
  integralGain: z
    .number()
    .positive('Integral gain must be positive')
...
// Tuning Queue Context Action Schema
const tuningQueueContextActionSchema = z.enum([
  'change_queue_id',
  'set_to_active',
  'remove_from_queue',
  'start_loop_analysis',
...
// Tuning Queue Context Option Schema
const tuningQueueContextOptionSchema = z.object({
  action: tuningQueueContextActionSchema,
...
// Tuning Queue Entry Schema
const tuningQueueEntrySchema = z.object({
  loopId: z
    .string()
    .min(1, 'Loop ID is required')
    .regex(
      /^[a-zA-Z0-9_-]+$/,
  ...
  queID: z.number().int('Queue ID must be an integer').positive('Queue ID must be positive'),
  isFocus: z.boolean(),
...
// Tuning Queue Action Type Schema
const tuningQueueActionTypeSchema = z.enum([
  'ADD_TO_QUEUE',
  'REMOVE_FROM_QUEUE',
  'UPDATE_QUEUE_ID',
  'SET_FOCUS_LOOP',
...
// Tuning Queue Action Schema
const tuningQueueActionSchema = z.object({
  type: tuningQueueActionTypeSchema,
  payload: z.record(z.string(), z.unknown()),
  timestamp: z.date(),
  userId: z.string().optional(),
});

// Tuning Queue State Schema
const tuningQueueStateSchema = z.object({
  entries: z.array(tuningQueueEntrySchema),
  focusLoopId: z.string().nullable(),
  maxQueueSize: z
    .number()
    .int('Max queue size must be an integer')
    .positive('Max queue size must be positive')
    .max(50, 'Max queue size cannot exceed 50'),
  nextAvailableQueID: z
    .number()
    .int('Next available queue ID must be an integer')
    .positive('Next available queue ID must be positive'),
  lastUpdated: z.date(),
});

// Keyboard Navigation State Schema
const keyboardNavigationStateSchema = z.object({
  isNavigationActive: z.boolean(),
  currentIndex: z
    .number()
    .int('Current index must be an integer')
    .min(0, 'Current index cannot be negative'),
  totalEntries: z
    .number()
    .int('Total entries must be an integer')
    .min(0, 'Total entries cannot be negative'),
  navigationMode: z.enum(['queue', 'parameters']),
});

// Parameter Validation Result Schema
const parameterValidationResultSchema = z.object({
  isValid: z.boolean(),
  errors: z.array(z.string()),
  warnings: z.array(z.string()),
  fieldErrors: z.record(z.string(), z.string()),
  suggestedValues: focusLoopEditableParametersSchema.partial().optional(),
});

// Tuning Interface Event Type Schema
const tuningInterfaceEventTypeSchema = z.enum([
  'PARAMETER_CHANGED',
  'QUEUE_UPDATED',
  'FOCUS_CHANGED',
  'ANALYSIS_STARTED',
...
// Tuning Interface Event Schema
const tuningInterfaceEventSchema = z.object({
  type: tuningInterfaceEventTypeSchema,
  loopId: z.string(),
  timestamp: z.date(),
  data: z.record(z.string(), z.unknown()),
  source: z.enum(['user', 'ai', 'websocket', 'system']),
});
...
// Section 1: Control Loop Tuning Interface Schema Exports
export type TuningMethod = z.infer<typeof tuningMethodSchema>;
...
export type TuningQueueContextOption = z.infer<typeof tuningQueueContextOptionSchema>;
export type TuningQueueEntry = z.infer<typeof tuningQueueEntrySchema>;
export type TuningQueueActionType = z.infer<typeof tuningQueueActionTypeSchema>;
export type TuningQueueAction = z.infer<typeof tuningQueueActionSchema>;
...
export type TuningInterfaceEventType = z.infer<typeof tuningInterfaceEventTypeSchema>;
export type TuningInterfaceEvent = z.infer<typeof tuningInterfaceEventSchema>;
