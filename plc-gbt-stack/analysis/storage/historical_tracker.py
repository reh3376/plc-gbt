...
from .database_schema import DatabaseSchemaManager, AnalysisType
from .result_storage import QueryFilter, AnalysisResultStorage
        ...
        try:
            with self.Session() as session:
                ...
                query = sa.select(
                    ...
                    (sa.func.count(sa.case((table.c.success == True, 1))) / 
                     sa.func.count().cast(sa.Float)).label('success_rate'),
                    sa.func.count(sa.case((table.c.success == False, 1))).label('error_count')
                ).where(
