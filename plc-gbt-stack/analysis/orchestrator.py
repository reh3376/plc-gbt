...
from .core.framework import (
    AnalysisFramework, AnalysisResult, AnalysisConfiguration, 
    AnalysisObjective, AnalysisStatus, analysis_framework
)
from .core.data_pipeline import (
    DataPipeline, ProcessingOptions, DataSource, DataSourceType,
    ProcessingResult, create_default_processing_options
)
from .core.caching import (
    CacheManager, CacheResult, cache_manager
)
from .core.job_manager import (
    AnalysisJobManager, AnalysisJob, JobConfiguration, 
    JobPriority, JobStatus, create_job_manager
)
...
class AnalysisOrchestrator:
    ...
    async def _execute_workflow(self, workflow: AnalysisWorkflow) -> WorkflowResult:
        ...
        try:
            ...
            if workflow.configuration.workflow_type == WorkflowType.SINGLE_ANALYSIS:
                result = await self._execute_single_analysis_workflow(workflow)
            elif workflow.configuration.workflow_type == WorkflowType.BATCH_ANALYSIS:
                result = await self._execute_batch_analysis_workflow(workflow)
            elif workflow.configuration.workflow_type == WorkflowType.COMPARATIVE_ANALYSIS:
                result = await self._execute_comparative_analysis_workflow(workflow)
            else:
    ...
    async def _execute_single_analysis_workflow(self, workflow: AnalysisWorkflow) -> Dict[str, Any]:
        ...
        analysis_config = AnalysisConfiguration(
            ...
            **{k: v for k, v in data_source.columns.items()}
        )
