#!/usr/bin/env node
/**
 * HTTP MCP Server - Fixed Implementation
...
export function loadAuthToken(): string | null {
  ...
  if (process.env.AUTH_TOKEN_FILE) {
    try {
      ...
      logger.info(
        `Loaded AUTH_TOKEN from file: ${process.env.AUTH_TOKEN_FILE}`
      );
      return token;
    } catch (error) {
      logger.error(
        `Failed to read AUTH_TOKEN_FILE: ${process.env.AUTH_TOKEN_FILE}`,
        error
      );
      console.error(
        `ERROR: Failed to read AUTH_TOKEN_FILE: ${process.env.AUTH_TOKEN_FILE}`
      );
...
function validateEnvironment() {
  ...
  if (!authToken || authToken.trim() === '') {
    ...
    console.error(
      'ERROR: AUTH_TOKEN is required for HTTP mode and cannot be empty'
    );
    console.error(
      'Set AUTH_TOKEN environment variable or AUTH_TOKEN_FILE pointing to a file containing the token'
    );
  ...
  if (authToken === 'REPLACE_THIS_AUTH_TOKEN_32_CHARS_MIN_abcdefgh') {
    logger.warn(
      '⚠️ SECURITY WARNING: Using default AUTH_TOKEN - CHANGE IMMEDIATELY!'
    );
...
export async function startFixedHTTPServer() {
  ...
  // Test tools endpoint
  app.get('/test-tools', async (req: Request, res: Response) => {
    try {
  ...
  expressServer = app.listen(port, host, () => {
    ...
    if (authToken === 'REPLACE_THIS_AUTH_TOKEN_32_CHARS_MIN_abcdefgh') {
      setInterval(() => {
        ...
        if (process.env.MCP_MODE === 'http') {
          console.warn(
            '⚠️ REMINDER: Still using default AUTH_TOKEN - please change it!'
          );
        }
