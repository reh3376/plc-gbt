...
export async function createDatabaseAdapter(dbPath: string): Promise<DatabaseAdapter> {
  ...
  try {
    ...
    if (
      ...
      if (process.env.MCP_MODE !== 'stdio') {
        logger.warn(
          `Node.js version mismatch detected. Better-sqlite3 was compiled for a different Node.js version.`
        );
      }
      if (process.env.MCP_MODE !== 'stdio') {
    ...
    try {
      ...
      if (process.env.MCP_MODE !== 'stdio') {
        logger.info(
          'Successfully initialized sql.js adapter (pure JavaScript, no native dependencies)'
        );
      }
      return adapter;
    } catch (sqlJsError) {
...
async function createSQLJSAdapter(dbPath: string): Promise<DatabaseAdapter> {
  ...
  try {
    ...
    throw new Error(
      'sql.js module not found. This might be an issue with npm package installation.'
    );
  }
