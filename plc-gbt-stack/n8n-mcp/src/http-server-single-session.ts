...
import { detectBaseUrl, formatEndpointUrls, getStartupBaseUrl } from './utils/url-detector';
...
export class SingleSessionHTTPServer {
  ...
  private validateEnvironment(): void {
    ...
    if (!this.authToken || this.authToken.trim() === '') {
      const message =
        'No authentication token found or token is empty. Set AUTH_TOKEN environment variable or AUTH_TOKEN_FILE pointing to a file containing the token.';
  ...
  async start(): Promise<void> {
    ...
    app.post('/mcp', async (req: Request, res: Response, _next: NextFunction): Promise<void> => {
