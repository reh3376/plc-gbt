...
export class NodeSpecificValidators {
  ...
  private static validateSlackDeleteMessage(context: NodeValidationContext): void {
    ...
    warnings.push({
      ...
      suggestion:
        'Consider archiving or updating the message instead if you need to preserve history',
    });
  }
  ...
  private static validateGoogleSheetsRead(context: NodeValidationContext): void {
    ...
    if (!config.options?.dataStructure) {
      suggestions.push(
        'Consider setting options.dataStructure to "object" for easier data manipulation'
      );
    }
  }
      ...
      case 'delete':
        if (!config.query || config.query === '{}') {
          errors.push({
            ...
            property: 'query',
            message:
              'Delete without query would remove all documents - this is a critical security issue',
  ...
  private static validateSQLQuery(
    ...
    if (query.includes('${') || query.includes('{{')) {
      warnings.push({
        ...
        suggestion:
          'Use parameterized queries with query parameters instead of string interpolation',
      });

      suggestions.push(
        'Example: Use "SELECT * FROM users WHERE id = $1" with queryParams: [userId]'
      );
    }
    ...
    if (lowerQuery.includes('drop')) {
      errors.push({
        ...
        property: 'query',
        message:
          'DROP operations are extremely dangerous and will permanently delete database objects',
  ...
  static validateHttpRequest(context: NodeValidationContext): void {
    ...
    if (config.retryOnFail) {
      ...
      if (
        ...
        (!config.maxTries || config.maxTries > 3)
      ) {
        warnings.push({
          type: 'best_practice',
          property: 'maxTries',
  ...
  private static validateJavaScriptCode(
    ...
    if ((functionWithAwait.test(code) || arrowWithAwait.test(code)) && !code.includes('async')) {
      warnings.push({
        ...
        suggestion:
          'Add async keyword to the function, or use top-level await (Code nodes support it)',
      });
    }
    ...
    if (code.includes('$helpers.httpRequest')) {
      suggestions.push(
        '$helpers.httpRequest is async - use: const response = await $helpers.httpRequest(...)'
      );
    }
  ...
  private static validatePythonCode(
    ...
    const unavailableImports = [
      {
        module: 'requests',
        suggestion: 'Use JavaScript Code node with $helpers.httpRequest for HTTP requests',
      },
      {
        module: 'pandas',
        suggestion: 'Use built-in list/dict operations or JavaScript for data manipulation',
      },
  ...
  private static validateReturnStatement(
    ...
    if (language === 'javaScript') {
      ...
      if (/return\s+{(?!.*\[)[\s\S]*}\s*;?$/.test(code) && !code.includes('json:')) {
        errors.push({
          type: 'invalid_value',
          property: 'jsCode',
    ...
    if (language === 'javaScript') {
      ...
      if (code.includes('$helpers') && !code.includes('typeof $helpers')) {
        warnings.push({
          ...
          suggestion:
            'Check availability first: if (typeof $helpers !== "undefined" && $helpers.httpRequest) { ... }',
        });
      }
      ...
      if (code.includes('$jmespath(') && /\$jmespath\s*\(\s*['"`]/.test(code)) {
        warnings.push({
          ...
          suggestion:
            'Use: $jmespath(dataObject, "query.path") not $jmespath("query.path", dataObject)',
        });
      }
      ...
      if (code.includes('items[0].json') && !code.includes('.json.body')) {
        ...
        if (
          ...
          warnings.push({
            ...
            suggestion:
              'Use items[0].json.body.fieldName instead of items[0].json.fieldName for webhook data',
          });
        }
  ...
  private static validateCodeSecurity(
    ...
    const dangerousPatterns = [
      ...
      {
        pattern: language === 'python' ? /exec\s*\(/ : /exec\s*\(/,
        message: "Avoid exec() - it's a security risk",
      },
    ...
    if (
      (code.includes('crypto.') || code.includes('randomBytes') || code.includes('randomUUID')) &&
      ...
      warnings.push({
        ...
        suggestion:
          'Add: const crypto = require("crypto"); at the beginning (ignore editor warnings)',
      });
    }
