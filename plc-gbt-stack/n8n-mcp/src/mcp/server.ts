...
export class N8NDocumentationMCPServer {
  ...
  private setupHandlers(): void {
    ...
    this.server.setRequestHandler(ListToolsRequestSchema, async () => {
      ...
      if (isConfigured) {
        ...
        logger.debug(
          `Tool listing: ${tools.length} tools available (${n8nDocumentationToolsFinal.length} documentation + ${n8nManagementTools.length} management)`
        );
      } else {
  ...
  private async getNodeInfo(nodeType: string): Promise<any> {
    ...
    const aiToolCapabilities = {
      ...
      environmentRequirement:
        node.package !== 'n8n-nodes-base' ? 'N8N_COMMUNITY_PACKAGES_ALLOW_TOOL_USAGE=true' : null,
    };
  ...
  private async searchNodesFuzzy(query: string, limit: number): Promise<any> {
    ...
    if (matchingNodes.length === 0) {
      const topScores = scoredNodes.sort((a, b) => b.score - a.score).slice(0, 5);
      logger.debug(
  ...
  private async searchNodesLIKE(query: string, limit: number): Promise<any> {
    ...
    const conditions = words
      .map(() => '(node_type LIKE ? OR display_name LIKE ? OR description LIKE ?)')
      .join(' OR ');
  ...
  private calculateRelevanceScore(node: NodeRow, query: string): number {
    ...
    // Special boost for common primary nodes
      ...
      (query_lower === 'http' || query_lower === 'http request' || query_lower === 'http call') &&
...
`,
  ...
  private async validateWorkflow(workflow: any, options?: any): Promise<any> {
    ...
    const validator = new WorkflowValidator(this.repository, EnhancedConfigValidator);
  ...
  private async validateWorkflowExpressions(workflow: any): Promise<any> {
    ...
    try {
      ...
      const expressionErrors = result.errors.filter(
        e => e.message.includes('Expression') || e.message.includes('$') || e.message.includes('{{')
      );

      const expressionWarnings = result.warnings.filter(
        w => w.message.includes('Expression') || w.message.includes('$') || w.message.includes('{{')
      );
