...
import { ExecutionStatus, McpToolResponse, WebhookRequest, Workflow } from '../types/n8n-api';
  ...
  try {
    ...
    return {
      ...
      data: {
        ...
        supportedN8nVersion,
        versionNote:
          'AI Agent: Please inform the user to verify their n8n instance version matches or is compatible with the supported version listed above. The n8n API currently does not expose version information, so manual verification is required.',
      },
    };
  } catch (error) {
    if (error instanceof N8nApiError) {
...
export async function handleDiagnostic(request: any): Promise<McpToolResponse> {
  ...
  const diagnostic: any = {
    ...
    troubleshooting: {
      ...
      documentation:
        'For detailed setup instructions, see: https://github.com/czlonkowski/n8n-mcp?tab=readme-ov-file#n8n-management-tools-optional---requires-api-configuration',
    },
  };
