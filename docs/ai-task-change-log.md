...
- **Non-blocking analyzer dispatch — [lines 355-358]**
  Uses `asyncio.to_thread(...)` to execute the synchronous analyzer without stalling the caller's event loop, keeping async services responsive during complex analyses.
- **Guide & summary hardening — [lines 426-455, 663-713]**
  Ensures output directories exist before writing, enforces UTF-8 file encoding, and triggers the `PRE_GENERATE_GUIDE` / `POST_GENERATE_GUIDE` plugin hooks around guide creation so extensions can add content safely.
...
- **Lifecycle primer update — `plc-gbt-stack/ai/docs/advanced/lifecycle-updates.md` [lines 12-32]**
  Notes the non-blocking analysis offload and clarifies how guide generation hooks fire so async agents know what guarantees exist.
- **Plugin guide expansion — `plc-gbt-stack/ai/docs/advanced/plugins.md` [lines 133-147]**
  Documents when the guide hooks trigger and the guarantees provided (UTF-8 output, resolved `Path`) for plugin authors.
- **API reference additions — `plc-gbt-stack/ai/docs/api-reference.md` [lines 52-88]**
