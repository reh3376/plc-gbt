...
class Still1SteamPIDAnalyzer:
    ...
    def load_and_preprocess_data(self):
        ...
        try:
            ...
            # Extract key control variables with cleaner names
            self.data['SP'] = self.data['still01-steam-pid.SP => (Aggregate=TimeAverage2)']  # Setpoint
            self.data['PV'] = self.data['still01-steam-pid.PV01-TIT4016 => (Aggregate=TimeAverage2)']  # Process Variable
            self.data['CV'] = self.data['still01-steam-pid.CV-FCV4054 => (Aggregate=TimeAverage2)']  # Control Variable
    ...
    def fit_fopdt_model(self, step_data=None):
        ...
        print(f"   Process Gain (Kp): {process_gain:.3f} °F/%")
        print(f"   Time Constant (τ): {time_constant:.2f} minutes")
        print(f"   Dead Time (θ): {dead_time:.2f} minutes")
        print(f"   Model: G(s) = {process_gain:.3f} / ({time_constant:.2f}s + 1) * e^(-{dead_time:.2f}s)")
    ...
    def calculate_pid_parameters(self):
        ...
        λ_time = lambda_factor * θ_sec
        
        if λ_time > 0:
            lambda_kp = τ_sec / (K * (λ_time + θ_sec))
            lambda_ti = τ_sec
    ...
    def analyze_current_performance(self):
        ...
        metrics = {
            ...
            'settling_time': self.estimate_settling_time(),
    ...
    def estimate_settling_time(self):
        """Estimate settling time from largest disturbance."""
        try:
            # Find largest deviation from setpoint
            error = np.abs(self.data['Error'])
            max_error_idx = np.argmax(error)
            
            # Look for when error settles within 2% of setpoint
            settling_threshold = 0.02 * self.data['SP'].iloc[0]  # 2% of setpoint
            
            # Find settling time after largest disturbance
            post_disturbance = error[max_error_idx:]
            settling_indices = np.where(post_disturbance < settling_threshold)[0]
            
            if len(settling_indices) > 0:
                settling_time = self.data['Time'].iloc[max_error_idx + settling_indices[0]] - self.data['Time'].iloc[max_error_idx]
                return max(settling_time, 0)
            else:
                return np.nan
        except:
            return np.nan
    ...
    def calculate_overshoot(self):
        ...
        try:
            ...
            overshoot_high = max(0, (max_pv - sp) / sp * 100)
            overshoot_low = max(0, (sp - min_pv) / sp * 100)
            
            return max(overshoot_high, overshoot_low)
        except:
            return 0
    
    def generate_stability_analysis(self):
        """Perform stability analysis for recommended PID parameters."""
        print("\n🔍 Phase 6: Stability Analysis")
        print("-" * 50)
        
        # Get recommended tuning parameters
        methods, recommended = self.calculate_pid_parameters()
        params = methods[recommended]
        
        # Calculate stability margins using simplified analysis
        K = self.process_model['gain']
        ...
        # Open loop gain at crossover frequency (simplified)
        # For FOPDT system: |G(jω)H(jω)| at phase crossover
        
        # Estimate crossover frequency
        ωc = 1 / τ  # Rough estimate
        
        # Phase margin calculation (simplified)
        phase_lag_process = -np.arctan(ωc * τ) - ωc * θ
        phase_lag_controller = -np.arctan(ωc * Ti)
        total_phase_lag = phase_lag_process + phase_lag_controller
        phase_margin = 180 + np.degrees(total_phase_lag)
        
        # Gain margin (simplified)
        gain_at_phase_crossover = K * Kp / np.sqrt(1 + (np.pi * τ / θ)**2)
        gain_margin = 1 / gain_at_phase_crossover if gain_at_phase_crossover > 0 else float('inf')
        gain_margin_db = 20 * np.log10(gain_margin) if gain_margin > 0 else float('inf')
        
        print(f"✅ Stability Analysis (Recommended: {recommended}):")
        print(f"   Phase Margin: {phase_margin:.1f}° (>45° recommended)")
        print(f"   Gain Margin: {gain_margin_db:.1f} dB (>6 dB recommended)")
        
        # Stability assessment
        if phase_margin > 45 and gain_margin_db > 6:
            stability = "Stable"
        elif phase_margin > 30 and gain_margin_db > 3:
            stability = "Marginal"
        else:
    ...
    def create_visualization(self):
        ...
        fig.suptitle('Still-1-Steam-PID Control Analysis Results', fontsize=16, fontweight='bold')
        
        # Plot 1: Process Variables vs Time
        ax1 = axes[0, 0]
        ax1.plot(self.data['Time'], self.data['SP'], 'r--', label='Setpoint (SP)', linewidth=2)
        ax1.plot(self.data['Time'], self.data['PV'], 'b-', label='Process Variable (PV)', linewidth=1)
        ...
        # Plot 2: Control Variable vs Time
        ax2 = axes[0, 1]
        ax2.plot(self.data['Time'], self.data['CV'], 'g-', label='Control Variable (CV)', linewidth=1)
        ax2.set_xlabel('Time (minutes)')
        ax2.set_ylabel('Control Output (%)')
        ax2.set_title('Controller Output Signal')
        ax2.legend()
        ax2.grid(True, alpha=0.3)
        
        # Plot 3: Error Signal
        ax3 = axes[1, 0]
        ax3.plot(self.data['Time'], self.data['Error'], 'r-', label='Control Error', linewidth=1)
        ax3.axhline(y=0, color='k', linestyle='--', alpha=0.5)
        ax3.set_xlabel('Time (minutes)')
        ax3.set_ylabel('Error (°F)')
        ax3.set_title('Control Error Signal')
        ax3.legend()
        ax3.grid(True, alpha=0.3)
        
        # Plot 4: PID Tuning Comparison
        ax4 = axes[1, 1]
        methods = ['Ziegler_Nichols', 'Cohen_Coon', 'Lambda_Tuning', 'Industrial_Temperature']
        kp_values = [self.pid_recommendations[m]['Kp'] for m in methods if m in self.pid_recommendations]
        ki_values = [self.pid_recommendations[m]['Ki'] for m in methods if m in self.pid_recommendations]
        
        x = np.arange(len(methods))
        width = 0.35
        
        ax4.bar(x - width/2, kp_values, width, label='Kp', alpha=0.8)
        ax4.bar(x + width/2, [ki*1000 for ki in ki_values], width, label='Ki×1000', alpha=0.8)
        ax4.set_xlabel('Tuning Method')
        ax4.set_ylabel('Parameter Value')
        ax4.set_title('PID Parameter Comparison')
        ax4.set_xticks(x)
        ax4.set_xticklabels([m.replace('_', ' ') for m in methods], rotation=45)
        ax4.legend()
        ax4.grid(True, alpha=0.3)
        
        plt.tight_layout()
        plt.savefig('still1_steam_pid_analysis.png', dpi=300, bbox_inches='tight')
        print("✅ Analysis plots saved as 'still1_steam_pid_analysis.png'")
