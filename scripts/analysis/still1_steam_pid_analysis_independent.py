...
class IndependentPIDAnalyzer:
    ...
    def analyze_current_performance(self):
        """Analyze current loop performance for independent PID"""
        print(f"\n" + "=" * 60)
        ...
        dt_data = (df['Time_min'].iloc[1] - df['Time_min'].iloc[0]) * 60  # seconds
        iae = np.sum(np.abs(error)) * dt_data / 60  # Convert to minutes
        ...
        print(f"  Mean Error: {mean_error:.3f}°F")
        print(f"  Error Std Dev: {std_error:.3f}°F ({error_rating})")
        print(f"  Max Absolute Error: {max_abs_error:.3f}°F")
        print(f"  RMSE: {rmse:.3f}°F")
        print(f"  IAE: {iae:.2f} °F⋅min")
        
        print(f"\nControl Effort Analysis:")
        print(f"  CV Standard Deviation: {cv_std:.2f}%")
        print(f"  CV Range: {cv_range:.1f}%")
        print(f"  CV Mean: {cv.mean():.1f}%")
        
        # Stability analysis
        settling_time = self.estimate_settling_time()
        print(f"\nDynamic Response:")
        print(f"  Estimated Settling Time: {settling_time:.1f} minutes")
    ...
    def estimate_settling_time(self):
        """Estimate settling time from step responses"""
        df = self.data
        cv_diff = np.abs(np.diff(df['CV']))
        step_threshold = np.percentile(cv_diff, 85)
        step_indices = np.where(cv_diff > step_threshold)[0]
        
        if len(step_indices) > 0:
            # Analyze first significant step
            step_idx = step_indices[0]
            step_end = min(step_idx + 50, len(df) - 1)  # Next 50 points
            
            pv_response = df['PV01'].iloc[step_idx:step_end]
            final_value = pv_response.iloc[-1]
            
            # Find 2% settling criterion
            settle_band = 0.02 * abs(final_value - pv_response.iloc[0])
            
            for i in range(10, len(pv_response)):
                if all(abs(pv_response.iloc[i:] - final_value) <= settle_band):
                    settle_time = (df['Time_min'].iloc[step_idx + i] - 
                                 df['Time_min'].iloc[step_idx])
                    return settle_time
    ...
    def generate_implementation_guide(self):
        ...
        if recommended_method in self.tuning_results:
            ...
            print(f"  Settling Time: <6 minutes")
            print(f"  Overshoot: <5% of setpoint")
            print(f"  Steady-state Error: ±1°F")
            print(f"  Control Effort: Smooth valve operation")
