...
def demo_custom_configurations():
    ...
    for config_name, config in configs.items():
        ...
        if "Temperature" in config_name:
            ...
            print(f"   • Very strict error tolerance (±0.2°C excellent)")
            print(f"   • Oscillation heavily weighted (40% of score)")
            print(f"   • Settling time critical for production")
            
        elif "Flow" in config_name:
            print(f"\n🌊 Flow Control Focus:")
            print(f"   • Percentage-based accuracy (MAPE = 50% of score)")
            print(f"   • More lenient oscillation limits (flow systems)")
            print(f"   • Control effectiveness emphasized")
            
        elif "Pressure" in config_name:
            print(f"\n⚠️ Pressure Control Focus:")
            print(f"   • Safety-critical configuration")
            print(f"   • MSE detects dangerous spikes")
            print(f"   • Extremely low oscillation tolerance")
            print(f"   • Fast emergency response required")
...
def analyze_with_custom_config():
    ...
    try:
        ...
        print(f"\n💡 Key Insights:")
        print(f"   • This configuration emphasizes oscillation control")
        print(f"   • Temperature-specific thresholds applied")
        print(f"   • Safety and stability prioritized over raw performance")
